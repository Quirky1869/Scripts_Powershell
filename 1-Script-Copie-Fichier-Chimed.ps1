#============================= Script pour copier des fichiers necessaires au bon fonctionnement de CHIMED =========================================#
...
$destinationWindows = (Get-ChildItem Env:\SystemRoot).value
$destinationPublicDesktop = (Get-ChildItem Env:\SystemDrive).value + '\Users\Public\Desktop'

# Paramétrages du Message Box OK
$ParamsOdbc = @{
    Content = "Il faut maintenant faire la base ODBC"
    ...
    ButtonType = 'None'
    CustomButtons = "D'accord"
    ContentTextForeground = 'Black'
    ButtonTextForeground = 'MidnightBlue'
  }

  # Paramétrages du Message Box Erreur

$ParamsErreur = @{
    Content = "Impossible de finaliser l'installation :("
    Title = "ERREUR"
    TitleFontSize = 20
    CornerRadius = 15
    TitleBackground = 'Red'
    TitleTextForeground = 'White'
    ButtonType = 'None'
    CustomButtons = "OK" , "Voir les logs"
    ContentTextForeground = 'Black'
    ButtonTextForeground = 'Red'
  }

    # Variable pour modifié la clé  de registre
    $oldPath = (Get-ItemProperty -Path ‘Registry::HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager\Environment’ -Name PATH).path
    $newPath = "I:\;$oldPath"


  # Chemin des logs
  $destinationLog = (Get-ChildItem Env:\USERPROFILE).value + "\Chimed.log"
  ...
  try {
    ...
    # Etape 1 : Copie des fichiers .ini
        Copy-Item -Path "\\sccm2012\SCCM_Repository$\Software\Chimed\fichiers INI chimed\CHIMCAT.INI" -Verbose -Force -Destination $destinationWindows
        Copy-Item -Path "\\sccm2012\SCCM_Repository$\Software\Chimed\fichiers INI chimed\CHIMED32.INI" -Verbose -Force -Destination $destinationWindows
        Copy-Item -Path "\\sccm2012\SCCM_Repository$\Software\Chimed\fichiers INI chimed\CHIMHAB.INI" -Verbose -Force -Destination $destinationWindows
        Copy-Item -Path "\\sccm2012\SCCM_Repository$\Software\Chimed\fichiers INI chimed\ChimTrt.INI" -Verbose -Force -Destination $destinationWindows
        Copy-Item -Path "\\sccm2012\SCCM_Repository$\Software\Chimed\fichiers INI chimed\ChwMig.ini" -Verbose -Force -Destination $destinationWindows

    # Etape 2 : Ajout dans le registre de Chimed Expect et Val
    
    reg import '\\sccm2012\SCCM_Repository$\Software\Chimed\registre chimed\CHIMED registre Except.reg'
    reg import '\\sccm2012\SCCM_Repository$\Software\Chimed\registre chimed\CHIMED registre Val.reg'
     
    # Etape 3 : Ajout de la variable I:\

    Set-ItemProperty -Path ‘Registry::HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager\Environment’ -Name PATH -Value $newPath -Verbose

    # Etape 4 : Rajout du dossier "Raccourci V5" sur le bureau
    
    Copy-Item -Path "\\sccm2012\SCCM_Repository$\Software\Chimed\Raccourcis V5" -Force -Recurse -Verbose -Destination $destinationPublicDesktop
    
    # Etape 5 : Message d'information pour ODBC

    New-WPFMessageBox @ParamsOdbc
    ...
    New-WPFMessageBox @ParamsErreur
    
    

        If ($WPFMessageBoxOutput -eq "Voir les logs"){

            Invoke-Item -Path $destinationLog
        }
