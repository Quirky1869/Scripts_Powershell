...
{
  param([Parameter(Mandatory=$true,ValueFromRemainingArguments=$true)][psobject[]]$letterList) #Definit que tout les parametres vont dans cette variable
  foreach($letter in $letterList)
  {
    $listePath.add($letter) #ajoute toute les lettre dans la liste des lecteurs a verifier
  }
}
function addDriveToList
{
  param([Parameter(Mandatory=$true,ValueFromRemainingArguments=$true)][psobject[]]$driveList) #definit que tout les parametres vont dans cette variable
  foreach($drive in $driveList)
  {
    $listeDrive.add($drive) #ajoute tout les chemins unc a la liste des chemin a monter
  }
}
addPathToList "g" "i" "j" "k"
addDriveToList "\\filer\forum$" "\\filer\apps$" "\\filer\adm$" "\\filer\medical$"
